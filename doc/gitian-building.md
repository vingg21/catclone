...
*Setup instructions for a gitian build of Cats Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the CATSCOIN
...
to the CATSCOIN GitHub release page.
...
- [Building Cats Coin](#building-catscoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/catscoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/catscoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/catscoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/catscoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/catscoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/catscoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/CATScoin/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@catscoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/catscoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/catscoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/catscoin-osx-*-build.assert
```
