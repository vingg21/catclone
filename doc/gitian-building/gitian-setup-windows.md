...
The build script we'll be using is contained in the CATSCOIN github repository ([contrib/gitian-build.py](https://github.com/CATScoin/catscoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the CATSCOIN repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/CATScoin/CATSCOIN/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, catscoin-detached-sigs, and catscoin GitHub repos.
