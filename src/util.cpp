...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "catscoin" is a composite category enabling all CATSCOIN-related debug output
            if (ptrCategory->count(std::string("catscoin"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\CATSCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\CATSCOIN
// Mac: ~/Library/Application Support/CATSCOIN
// Unix: ~/.catscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CATSCOIN";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "catscoin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty catscoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override catscoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "catscoind.pid"));
