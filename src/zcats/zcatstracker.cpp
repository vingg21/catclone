// Copyright (c) 2018-2019 The CATSCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zcats/deterministicmint.h>
#include "zcatstracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzCATSTracker::CzCATSTracker(std::string strWalletFile)
{
...
CzCATSTracker::~CzCATSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCATSTracker::Init()
{
...
bool CzCATSTracker::Archive(CMintMeta& meta)
{
...
bool CzCATSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCATSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCATSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCATSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzCATSTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzCATSTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzCATSTracker::GetSerialHashes()
{
...
CAmount CzCATSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCATSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCATSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCATSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCATSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCATSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCATSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCATSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCATSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCATSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCATSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCATSWallet* zCATSWallet)
{
    bool iszCATSWalletInitialized = (NULL != zCATSWallet);
    CMintMeta meta;
    ...
    if (! iszCATSWalletInitialized)
        zCATSWallet = new CzCATSWallet(strWalletFile);
    meta.isSeedCorrect = zCATSWallet->CheckSeed(dMint);
    if (! iszCATSWalletInitialized)
        delete zCATSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCATSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCATSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCATSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCATSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCATSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCATSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCATSWallet* zCATSWallet = new CzCATSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zCATSWallet);
        }
        delete zCATSWallet;
        LogPrint("zero", "%s: added %d dzcats from DB\n", __func__, listDeterministicDB.size());
    }
