...
CzCATSWallet::CzCATSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcats seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCATSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCATSSeed_deprecated()) {
                    LogPrintf("%s: Updated zCATS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcats seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcats wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCATSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCATSCount(nCountLastUsed);
    else if (!walletdb.ReadZCATSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCATSWallet::Lock()
{
    seedMaster = 0;
}

void CzCATSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCATSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCATS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCATSWallet::LoadMintPoolFromDB()
{
...
void CzCATSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCATSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCATSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcatsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCATSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCATS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcatsTracker which also adds to database
    pwalletMain->zcatsTracker->Add(dMint, true);
...
void CzCATSWallet::SeedToZCATS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCATSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCATSWallet::GenerateDeterministicZCATS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCATSWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCATSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCATSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
