...
public:
    ...
    bool WriteZCATSSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZCATSSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZCATSSeed_deprecated(uint256& seed);
    bool EraseZCATSSeed();
    bool EraseZCATSSeed_deprecated();

    bool WriteZCATSCount(const uint32_t& nCount);
    bool ReadZCATSCount(uint32_t& nCount);
