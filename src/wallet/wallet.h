...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zcatsspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CATS at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCATS send
enum ZerocoinSpendStatus {
    ZCATS_SPEND_OKAY = 0,                            // No error
    ZCATS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCATS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCATS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCATS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCATS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCATS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCATS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCATS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCATS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCATS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCATS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCATS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCATS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCATS_SPENT_USED_ZCATS = 14,                      // Coin has already been spend
    ZCATS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCATS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZCatsBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZCATSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzcatsAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzCATSTracker> zcatsTracker;
    ...
    void setZWallet(CzCATSWallet* zwallet);
    CzCATSWallet* getZWallet();
    bool isZeromintEnabled();
    void setZCatsAutoBackups(bool fEnabled);
    ...
    /// Get 10000 CATS output and keys which can be used for the Masternode
    ...
    /** zCATS reset */
    boost::signals2::signal<void()> NotifyzCATSReset;
