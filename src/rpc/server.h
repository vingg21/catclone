...
/**
 * CATSCOIN RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZcatsSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzcatsseed(const UniValue& params, bool fHelp);
extern UniValue getzcatsseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzcats(const UniValue& params, bool fHelp);
extern UniValue dzcatsstate(const UniValue& params, bool fHelp);
