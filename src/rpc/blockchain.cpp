...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zcatsObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zcatsObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zcatsObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZCATS()) ? "Not available" : std::to_string(
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in catscoin\n"
...
UniValue getblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"zCATSsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zCATS denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zCATS denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zCATS denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zCATS denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zCATS denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zCATS denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zCATS denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zCATS denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zCATS denominations\n"
            "  },\n"
            "  \"CoinStake\" :\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of catscoin addresses\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZCATS_FAILED_ACCUMULATOR_INITIALIZATION);
...
UniValue getblockindexstats(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
                ...
                "  }\n"
                "  \"txbytes\": xxxxx                (numeric) Sum of the size of all txes (zCATS excluded) over block range\n"
                "  \"ttlfee\": xxxxx                 (numeric) Sum of the fee amount of all txes (zCATS mints excluded) over block range\n"
                "  \"ttlfee_all\": xxxxx             (numeric) Sum of the fee amount of all txes (zCATS mints included) over block range\n"
