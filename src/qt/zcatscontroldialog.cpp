...
#include "ui_zcatscontroldialog.h"
...
std::set<std::string> ZCatsControlDialog::setSelectedMints;
std::set<CMintMeta> ZCatsControlDialog::setMints;

bool CZCatsControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZCatsControlDialog::COLUMN_DENOMINATION || column == ZCatsControlDialog::COLUMN_VERSION || column == ZCatsControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZCatsControlDialog::ZCatsControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCatsControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zCATS Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZCats->setProperty("cssClass", "text-main-purple");
    ui->labelZCats_int->setProperty("cssClass", "text-main-purple");
...
ZCatsControlDialog::~ZCatsControlDialog()
{
    delete ui;
}

void ZCatsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCatsControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZCatsControlWidgetItem* itemDenom(new CZCatsControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZCatsControlWidgetItem *itemMint = new CZCatsControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zcatsTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zcatsTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zCATS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCATS seed used to mint this zCATS is not the same as currently hold in the wallet";
            else
...
void ZCatsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCatsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCats_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZCatsControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCatsControlDialog::GetSelectedMints()
{
...
void ZCatsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
