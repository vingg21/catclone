...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::CATS);
    ...
    if (settings.contains("nAnonymizeCATScoinAmount") || reset)
        SoftSetArg("-anonymizecatscoinamount", settings.value("nAnonymizeCATScoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in catscoin.cpp during shotdown to also remove the window positions
