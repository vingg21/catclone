...
#include "qt/catscoin/dashboardwidget.h"
#include "qt/catscoin/forms/ui_dashboardwidget.h"
#include "qt/catscoin/sendconfirmdialog.h"
#include "qt/catscoin/txrow.h"
...
DashboardWidget::DashboardWidget(CATSCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of CATS staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareCats, "square-chart-cats");
    setCssProperty(ui->labelSquarezCats, "square-chart-zcats");
    setCssProperty(ui->labelCats, "text-chart-cats");
    setCssProperty(ui->labelZcats, "text-chart-zcats");
    ui->labelZcats->setVisible(false);
    ui->labelSquarezCats->setVisible(false);
    ...
    ui->labelAmountZcats->setText("0 zCATS");
    ui->labelAmountZcats->setVisible(false);
    ui->labelAmountCats->setText("0 CATS");
    setCssProperty(ui->labelAmountCats, "text-stake-cats-disable");
    setCssProperty(ui->labelAmountZcats, "text-stake-zcats-disable");
    ...
    ui->btnHowTo->setText(tr("How to get CATS"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("CATS")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isCats = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZCATS;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair CATS, zCATS
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            cats = (pair.first != 0) ? pair.first / 100000000 : 0;
            zcats = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalCats += pair.first;
            chartData->totalZcats += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesCats.append(cats);
        chartData->valueszCats.append(zcats);

        int max = std::max(cats, zcats);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("CATS");
    set1 = new QBarSet("zCATS");
    ...
    set0->append(chartData->valuesCats);
    set1->append(chartData->valueszCats);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalCats > 0 || chartData->totalZcats > 0) {
        setCssProperty(ui->labelAmountCats, "text-stake-cats");
        setCssProperty(ui->labelAmountZcats, "text-stake-zcats");
    } else {
        setCssProperty(ui->labelAmountCats, "text-stake-cats-disable");
        setCssProperty(ui->labelAmountZcats, "text-stake-zcats-disable");
    }
    forceUpdateStyle({ui->labelAmountCats, ui->labelAmountZcats});
    ui->labelAmountCats->setText(GUIUtil::formatBalance(chartData->totalCats, nDisplayUnit));
    ui->labelAmountZcats->setText(GUIUtil::formatBalance(chartData->totalZcats, nDisplayUnit, true));
