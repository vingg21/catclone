...
#include "qt/catscoin/topbar.h"
#include "qt/catscoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/catscoin/lockunlock.h"
#include "qt/catscoin/qtutils.h"
#include "qt/catscoin/receivedialog.h"
...
TopBar::TopBar(CATSCOINGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopCats, ui->labelAmountTopzCats}, "amount-small-topbar");
    setCssProperty({ui->labelAmountCats, ui->labelAmountzCats}, "amount-topbar");
    setCssProperty({ui->labelPendingCats, ui->labelPendingzCats, ui->labelImmatureCats, ui->labelImmaturezCats}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzCats->setVisible(false);
    ui->labelAmountzCats->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzCats->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezCats->setVisible(false);
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("CATSCOIN")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount catsAvailableBalance = balance + delegatedBalance;

    // zCATS Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalCats = GUIUtil::formatBalance(catsAvailableBalance, nDisplayUnit);
    QString totalzCats = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopCats->setText(totalCats);
    ui->labelAmountTopzCats->setText(totalzCats);

    // Expanded
    ui->labelAmountCats->setText(totalCats);
    ui->labelAmountzCats->setText(totalzCats);

    ui->labelPendingCats->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzCats->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureCats->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezCats->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
