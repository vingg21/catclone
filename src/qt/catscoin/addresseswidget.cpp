...
#include "qt/catscoin/forms/ui_addresseswidget.h"
#include "qt/catscoin/addresslabelrow.h"
#include "qt/catscoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(CATSCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a CATSCOIN address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress catsAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(catsAdd.Get(), label.toUtf8().constData(),
                catsAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
