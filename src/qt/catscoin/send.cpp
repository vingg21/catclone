...
#include "qt/catscoin/send.h"
#include "qt/catscoin/forms/ui_send.h"
#include "qt/catscoin/addnewcontactdialog.h"
#include "qt/catscoin/qtutils.h"
#include "qt/catscoin/sendchangeaddressdialog.h"
#include "qt/catscoin/optionbutton.h"
#include "qt/catscoin/sendconfirmdialog.h"
#include "qt/catscoin/myaddressrow.h"
#include "qt/catscoin/guitransactionsutils.h"
...
SendWidget::SendWidget(CATSCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("CATS");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zCATS");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (CATS)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a CATSCOIN address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 CATS");
    ...
    setCssProperty(coinIcon, "coin-icon-cats");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onCATSSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onCATSSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send CATS");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zCATS");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZcats = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZcats));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZcats ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendCats = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendCats ? AskPassphraseDialog::Context::Send_CATS : AskPassphraseDialog::Context::Send_zCATS, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendCats) ? send(recipients) : sendZcats(recipients)) {
...
bool SendWidget::sendZcats(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zCATS is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zCATS selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZCatsControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZCatsControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZcats(
        ...
        inform(tr("zCATS transaction sent!"));
        ZCatsControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZCATS_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zCATS require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zCATS transaction
        ...
        emit message("zCATS transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isCATS){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any CATS to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZCatsControlDialog *zCatsControl = new ZCatsControlDialog(this);
            zCatsControl->setModel(walletModel);
            zCatsControl->exec();
            ui->btnCoinControl->setActive(!ZCatsControlDialog::setSelectedMints.empty());
            zCatsControl->deleteLater();
        } else {
            inform(tr("You don't have any zCATS in your balance to select."));
        }
    }
}
...
void SendWidget::onCATSSelected(bool _isCATS){
    isCATS = _isCATS;
    setCssProperty(coinIcon, _isCATS ? "coin-icon-cats" : "coin-icon-zcats");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress catsAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(catsAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(catsAdd.Get(), dialog->getLabel().toStdString(), "send")) {
