...
#include "qt/catscoin/privacywidget.h"
#include "qt/catscoin/forms/ui_privacywidget.h"
#include "qt/catscoin/qtutils.h"
#include "guiutil.h"
#include "qt/catscoin/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(CATSCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zCATS anonymizes your CATS by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zCATS or convert back to CATS"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 CATS ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zCATS");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zCATS");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zCATS");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zCATS");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zCATS");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zCATS");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zCATS");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zCATS");
    ...
    // Only Convert to CATS enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzCATS->setTitleClassAndText("btn-title-grey", "Total 0 zCATS");
    ui->btnTotalzCATS->setSubTitleClassAndText("text-subtitle", "Show denominations of zCATS owned.");
    ui->btnTotalzCATS->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select CATS outputs to mint into zCATS.");
    ...
    connect(ui->btnTotalzCATS, SIGNAL(clicked()), this, SLOT(onTotalZcatsClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zCATS");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of CATS to mint into zCATS"));
    }else{
        btnText = tr("Convert back to CATS");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zCATS to convert back into CATS"));
    }
...
void PrivacyWidget::onTotalZcatsClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzCATS->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzCATS->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zCATS is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zCATS, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zCATS").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zCATS minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZcats(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zCATS converted back to CATS"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCATS </b>";
    ...
    ui->btnTotalzCATS->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
